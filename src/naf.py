...
class NAF(object):
    def __init__(self, sess,
        ...
        with tf.name_scope('optimizer'):
            ...
            self.loss = tf.reduce_mean(tf.squared_difference(
                self.target_y, tf.squeeze(self.pred_network.Q)), name='loss')
    ...
    def run(self, monitor=False, display=False, is_train=True):
        ...
        if monitor:
            self.env.monitor.start('/tmp/%s-%s' %
                                   (self.stat.env_name, get_timestamp()))

        report_steps = self.max_steps * self.update_repeat
        for self.idx_episode in range(self.max_episodes):
            ...
            for t in range(0, self.max_steps):
                ...
                if terminal:
                    self.strategy.reset()
                    break
            print("Eps: %i|TotalR: %.2f|AvgR: %.2f|AvgQ: %.2f|AvgV: %.2f|AvgA: %.2f|AvgLoss: %.2f" %
                  (self.idx_episode + 1, episode_reward, episode_reward / report_steps, sum_q / report_steps, sum_v / report_steps, sum_a / report_steps, sum_l / report_steps))
    ...
    def run2(self, monitor=False, display=False, is_train=True):
        ...
        loss = tf.reduce_mean(tf.squared_difference(
            target_y, tf.squeeze(self.pred_network.Q)), name='loss')
                    ...
                    for k in range(self.update_repeat):
                        if len(prestates) > self.batch_size:
                            indexes = np.random.choice(
                                len(prestates), size=self.batch_size)
                        else:
                            indexes = range(len(prestates))

                        # Q-update
                        v_ = self.target_network.V.eval(
                            {self.target_network.x: np.array(poststates)[indexes]})
                        y_ = np.array(rewards)[indexes] + \
                            self.discount * np.squeeze(v_)

                        tmp1, tmp2 = np.array(prestates)[
                            indexes], np.array(actions)[indexes]
                        loss_ += l_
    ...
    def q_learning_minibatch(self):
        ...
        for iteration in range(self.update_repeat):
            if len(self.rewards) >= self.batch_size:
                indexes = np.random.choice(
                    len(self.rewards), size=self.batch_size)
            else:
