...
class Network:
    def __init__(self, sess, input_shape, action_size, hidden_dims,
        ...
        with tf.variable_scope(scope):
            # input states placeholder
            x = tf.placeholder(tf.float32, (None,) +
                               tuple(input_shape), name='observations')
            ...
            with tf.name_scope('hidden'):
                # if not share part of the network between different outputs (Not default)
                if use_seperate_networks:
                    ...
                    for scope in ['v', 'l', 'mu']:
                        with tf.variable_scope(scope):
                            ...
                            hid_outs[scope] = h
                # if share part of network for different outputs (default)
                else:
                    ...
                    # making multiple hidden layers using a for loop
                    for idx, hidden_dim in enumerate(hidden_dims):
                        h = fc(h, hidden_dim, is_train, hidden_w, weight_reg=w_reg,
                               activation_fn=hidden_fn, use_batch_norm=use_batch_norm, scope='hid%d' % idx)
            ...
            with tf.name_scope('advantage'):
                ...
                for idx in xrange(action_size):
                    ...
                    # non_diag_elements with the size count-1 (1 comes from diag element)
                    non_diag_elems = tf.slice(
                        l, (0, pivot + 1), (-1, count - 1))
            ...
            with tf.name_scope('optimization'):
                self.target_y = tf.placeholder(
                    tf.float32, [None], name='target_y')
                self.loss = tf.reduce_mean(tf.squared_difference(
                    self.target_y, tf.squeeze(Q)), name='loss')
    ...
    def make_soft_update_from(self, network, tau):
        ...
        for from_, to_ in zip(network.variables, self.variables):
            if 'BatchNorm' in to_.name:
                ...
                self.assign_op[to_.name] = to_.assign(
                    tau * from_ + (1 - tau) * to_)
